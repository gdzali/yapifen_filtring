...
foreach ($post_term_1 as $ust_yapi_term) {
  ...
  $slug_ust_yapi = $ust_yapi_term->slug;
}
foreach ($post_term_2 as $ulastirma_term) {
  $print_ulastirma = $ulastirma_term->name;
  $slug_ulastirma = $ulastirma_term->slug;
}
foreach ($post_term_3 as $su_enerji_term) {
  $print_enerji_term = $su_enerji_term->name;
  $slug_enerji_term = $su_enerji_term->slug;
}
foreach ($post_term_4 as $endustriyel_term) {
  $print_endustriyel = $endustriyel_term->name;
  $slug_endustriyel = $endustriyel_term->slug;
}

if ($print_ust_yapi != null) {
    $related_taxonomy = 'ust-yapi';
    $related_taxonomy_term_slug = $slug_ust_yapi;
} elseif ($print_ulastirma != null) {
    $related_taxonomy = 'ulastirma';
    $related_taxonomy_term_slug = $slug_ulastirma;
} elseif ($print_enerji_term != null) {
    $related_taxonomy = 'su-enerji';
    $related_taxonomy_term_slug = $slug_enerji_term;
} else {
    $related_taxonomy = 'endustriyel';
    $related_taxonomy_term_slug = $slug_endustriyel;
}
?>
<div style="margin-top: -60px" class="xts-site-content">
  <div class="container">
                          ...
                          <div class="first-image xts-image-gallery xts-row xts-row-1 xts-row-spacing-20 xts-textalign-left xts-items-start xts-photoswipe-images xts-lightbox-gallery xts-caption-lightbox xts-masonry-layout xts-autoplay-animations-off" data-animation-delay="" style="position: relative; height: 396.859px;">
            								<div class="xts-col" style="position: absolute; left: 0px; top: 0px;">
            										<figure class="xts-image">
            									       ...
            									       <img src="<?=  esc_url($image['sizes']['first-image-inner']); ?>" title="" alt="BUYUK">
            											</a>
            										</figure>
            								</div>
            							</div>
    ...
    <?php
   			xts_portfolio_template(
   					array(
   						'image_size'            => xts_get_loop_prop( 'related-slider' ),
              ...
              'taxonomies'            => $related_taxonomy,
              'taxonomy_term'         => $related_taxonomy_term_slug,
              'orderby'               => 'rand',
              'custom_project'        => true,
